...
directory = '/home/Imatge/hd/datasets/instaBarcelona/img/'
get_videos = False
first = True
...
while True:
    ...
    remaining_num_2_download = num_2_download - len(os.listdir(directory))
    if remaining_num_2_download < 0: remaining_num_2_download = 0

    print "Number of images for " + query + ': ' + str(len(os.listdir(directory))) + "  --  Remaining: " + str(remaining_num_2_download)

    if remaining_num_2_download < 1:
        continue

    looter = InstaLooter(directory=directory, hashtag=query,
