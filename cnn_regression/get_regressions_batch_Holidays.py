...
with open('../../../datasets/EmotionDataset/gt_filtered/test.txt') as f:
    test = f.readlines()


#Model name
model = 'EmotionDataset_contrastive_softNegativeBatch_m01_iter_316000'

#Output file
output_file_dir = '../../../datasets/EmotionDataset/regression_output/' + model
...
# load net
net = caffe.Net('../googlenet_regression/prototxt/deploy.prototxt', '../../../datasets/EmotionDataset/models/saved/'+ model + '.caffemodel', caffe.TEST)
...
while i < len(test):
    ...
    for x in range(0, batch_size):
        ...
        # load image
        filename = '../../../datasets/EmotionDataset/img/' + str(test[i].split(',')[0])
        im = Image.open(filename)
        im_o = im
        im = im.resize((size, size), Image.ANTIALIAS)
        indices.append(str(test[i].split(',')[0]))
