...
def build_inception_v4(split, num_classes, batch_size, resize_w, resize_h, crop_w=0, crop_h=0, crop_margin=0, mirror=0, rotate=0, HSV_prob=0, HSV_jitter=0, train=True, deploy=False):
    ...
    n.classifier_C = L.InnerProduct(n.pool_8x8_s1_drop, num_output=num_classes,
                                  param=[dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)],
    ...
    if not deploy:
        n.loss = L.SigmoidCrossEntropyLoss(n.classifier_C, n.label)

    if deploy:
        n.probs = L.Sigmoid(n.classifier_C)
